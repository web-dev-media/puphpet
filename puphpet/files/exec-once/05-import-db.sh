...
#path='/srv/databases/'
cd '/vagrant/assets/data/databases/'
...
then
	...
	do
		...
		db_exist=`mysql -u dbuser -p123 --skip-column-names -e "$mysql_cmd"`

		if [ "$?" != "0" ]
		then
			echo "create database $pre_dot" | mysql -u dbuser -p123
			mysql -u dbuser -p123 $pre_dot < $pre_dot.sql
		fi
